...
HTTPServer.prototype.addRoute = function addRoute(template, handler){
	this.routes.addTemplate(template, {}, handler);
}
...
Conneg.prototype.index = function index(routes){
	var list = [];
...
Conneg.prototype.prepare = function prepare(route, euri, queryMap){
...
First.prototype.listing = function listing(routes){
	...
	})).then(function(lisiting){
		var list = [];
		lisiting.forEach(function(v){
			if(list.indexOf(v)<0) list.push(v);
		});
