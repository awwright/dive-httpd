...
HTTPServer.prototype.handleRequest = function handleRequest(req, res){
	...
	if(req.method=='GET' || req.method==='HEAD' || req.method=='POST' || req.method=='PATCH' || req.method=='DELETE'){
		resourceRequest = app.prepare(uriHier, euri, queryMap, req);
	}else if(req.method=='PUT'){
		resourceRequest = app.store(req, res);
	}else{
		// TODO: var resource = await app.prepare(uri);
		// Check if req.method is in resource.methods
		// If so, call resource.callMethod(req) (for e.g. LOCK)
		// If not, then try: this.callMethod(req.method, uri) (for e.g. MKCOL)
		// Failing all else, then return 501
		resourceRequest = app.error(uriHier, {statusCode: 501});
	}
