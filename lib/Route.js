...
Route.prototype.matchUri = function matchUri(uri){
	...
	if(typeof uri==='string'){
		...
		// - routerURITemplate is the string URI Template in question
		// - uriTemplateRouter is an instance of Router that resolves only the routerURITemplate
		if(!this.uriTemplateRouter){
			this.uriTemplateRouter = new TemplateRouter.Router();
			this.uriTemplateRouter.addTemplate(this.routerURITemplate);
		}
