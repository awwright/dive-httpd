...
const ResponseMessage = require('./ResponseMessage.js').ResponseMessage;
...
function Route(opts){
	...
	if(opts && typeof opts=='object'){
		...
		if(typeof opts.innerRoute=='object') this.innerRoute = opts.innerRoute;
...
// Reject if there is a configuration problem.
	...
	if(opts && typeof opts=='object'){
		...
		if(opts.match){
			...
			if(typeof opts.match.uriTemplate !== 'string'){
				throw new Error('Expected opts.match.uriTemplate to be a string');
			}
		...
		if(opts.error){
			if(typeof opts.error !== 'function'){
				throw new Error('Expected opts.error to be a function');
			}
...
Resource.prototype.handle = function handle(req){
	...
	function emitError(err){
		...
		resource.error(req.uri, err).then(function(errRsc){
			if(errRsc){
				...
				if(res.readableSide.listenerCount('error')) res.readableSide.emit('error', err);
				else throw err;
			}
		}).catch(function(err){
	...
	if(req.method==='GET' || req.method==='HEAD'){
		...
		stream = resource.post(req);
	}else if(req.method==='PATCH'){
		// TODO first see if there's a native PATCH, else GET then PUT
		stream = resource.patch(req);
	}else if(req.method==='DELETE'){
		stream = resource.del(req);
	}else{
		stream = resource.custom(req);
	}
	if(!stream){
		process.nextTick(function(){
			res.clientReadableSide.emit('error', new Error('Resource did not return a response stream'));
		});
		return;
	}
	...
	stream.on('error', function(err){ res.clientReadableSide.emit('error', err); });
...
Resource.prototype.renderString = function renderString(req){
	return ResponseMessage.fromStream(this.render(req));
};
