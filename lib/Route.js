...
Resource.prototype.handle = function handle(req){
		...
		if(resource.error){
			resource.error(req.uri, err).then(function(errRsc){
				if(errRsc){
					...
					if(res.listenerCount('error')) res.emit('error', err);
					else throw err;
				}
			}).catch(function(err){
				if(res.listenerCount('error')) res.emit('error', err);
	...
	if(!stream){
		process.nextTick(function(){
			res.emit('error', new Error('Resource did not return a response stream'));
		});
		return;
	}
