...
function TransformRoute(opts, innerRoute){
	...
	if(opts && typeof opts.renderTransform === 'function'){
		this.renderTransform = opts.renderTransform;
	}else if(opts.renderTransform){
		throw new Error('options.renderTransform must be a function');
	}
...
TransformRoute.prototype.render = function render(resource, req){
	...
	this.renderTransform(resource, req, input, res.writableSide).catch(function(error){
		res.readableSide.emit('error', error);
	});
