...
function TransformRoute(opts, innerRoute){
	...
	if(this.uriTemplateRoute===undefined) this.uriTemplateRoute = this.innerRoute.uriTemplateRoute;
...
TransformRoute.prototype.watch = function watch(cb){
	const route = this;
	if(!this.uriTemplateRoute) throw new Error('Unknown Route');
	return this.innerRoute.watch(function(inner, ancestor){
		var match = inner.match.rewrite(route.uriTemplateRoute);
