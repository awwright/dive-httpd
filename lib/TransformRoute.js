...
function TransformRoute(opts, innerRoute){
	...
	if(this.uriRoute===undefined) this.uriRoute = this.innerRoute.uriRoute;
	if(this.contentType===undefined) this.contentType = this.innerRoute.contentType;
	if(!this.uriRoute) throw new Error('Expected `uriRoute`');
}
...
TransformRoute.prototype.watch = function watch(cb){
	const route = this;
	if(!this.uriRoute) throw new Error('Unknown Route');
	return this.innerRoute.watch(function(inner, ancestor){
		var match = inner.match.rewrite(route.uriRoute);
...
TransformRoute.prototype.render = function render(resource, req){
	...
	input.on('error', (err)=>res.destroy(err));
	if(input.headersReady) input.headersReady.then(headersReady);
	else headersReady();
	function headersReady(){
		self.render_transform(resource, req, input, res.writableSide).catch(function(error){
			res.destroy(error);
		});
	}
