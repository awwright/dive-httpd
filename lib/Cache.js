...
function Cache(opts, innerRoute){
	if(!(this instanceof Cache)) return new Cache(opts, innerRoute);
	Route.call(this, {innerRoute});
	...
	this.uriRoute = this.innerRoute.uriRoute;
...
function parseCacheControl(_value){
	...
	while(value.length){
		...
		if(value[0]==='='){
			...
			map.set(directiveName.toLowerCase(), directiveValue);
		}else{
			map.set(directiveName.toLowerCase(), true);
		}
		// Consume whitespace
		{
			const i = value.match(/^\s*/);
...
CacheObject.prototype.setHeadersOn = function(dst){
...
Cache.prototype.computeFreshnessLifetime = function computeFreshnessLifetime(req, res){
	...
	const sharedMaxAgeStr = resCacheControl.has('s-maxage') && resCacheControl.get('s-maxage');
	if(this.private === false && sharedMaxAgeStr && sharedMaxAgeStr.match(/^[0-9]*$/)){
	...
	if(maxAgeStr && maxAgeStr.match(/^[0-9]*$/)){
	...
	if(typeof res.headers['last-modified']==='string' && res.statusCode===200){
		...
		if(resLastModified){
			// 8% of the time since the document was last modified
			const duration = Date(res.headers['last-modified']) - Date();
			if(duration >= 0) return Math.floor(duration / 1000 * 0.08);
		}
	}
};
...
ResourceCacheHit.prototype.handle = function handle(req){
	...
	const response_is_fresh = (current_age < obj.freshness_lifetime);
	...
	if(
		...
		if(response_is_fresh){
			const etag = typeof obj.res.headers['etag']==='string' && obj.res.headers['etag'].match(/^(W\/)?"(.*)"$/) ? obj.res.headers['etag'] : null;
			if(obj.res.statusCode===200 && etag && req.headers['if-none-match']===etag && etag[0]!=='W'){
				res.statusCode = 304;
				res.statusMessage = 'Not Modified';
				[ 'Date', 'Cache-Control', 'Content-Location', 'ETag', 'Expires', 'Vary' ].forEach(function(name){
					if(obj.res.hasHeader(name)) res.setHeader(name, obj.res.getHeader(name));
				});
				res.end();
			}else{
				obj.setHeadersOn(res);
...
ResourceCacheMiss.prototype.handle = function handle(req){
	...
	const route = this.route;
	const key = this.key;
	const downstream = new ResponsePassThrough;
	const res = this.inner.handle(req);
	res.once('error', (err)=>downstream.destroy(err));
	res.headersReady.then(async function(){
		if(res.statusCode === 206) return;
		...
		if(
			...
			// Don't store responses with no-cache because we can never use these
			&& !resCacheControl.has('no-cache')
			// If a shared cache, do not store requests with Authorization
			&& (route.private ? true : !res.headers['authorization'])
			&& (route.private ? true : !resCacheControl.has('private'))
			...
			// Store the result in the cache, then serve it from the cache
			// This ensures that the next serve from the cache will be identical to this one
			await route.storageWrite(key, req, res);
			const hit = await route.storageRead(key);
			hit.res.pipeHeaders(downstream);
			downstream.end(hit.payload);
			// FIXME... this error should cause a 500, not a timeout
			// throw new Error('Test');
		}else{
			res.pipeMessage(downstream);
		}
	}).catch(function(err){
		downstream.destroy(err);
	});
	return downstream.clientReadableSide;
};
