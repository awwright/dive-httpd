...
Cache.prototype.computeFreshnessLifetime = function computeFreshnessLifetime(req, res){
	...
	const resCacheControl = (function(){
		try{
			return parseCacheControl((res.headers['cache-control'] || '').toString());
		}catch(_){
			return new Map;
		}
	})();
...
Cache.prototype.computeAge_s = function computeAge_s(sreq, sres, req, sresDate, now){
	...
	return storedAge + ((now || new Date) - sresDate)/1000;
};
...
ResourceCacheMiss.prototype.handle = function handle(req){
	...
	copy.headersReady.then(function(){
		...
		try{
			var reqCacheControl = parseCacheControl((req.headers['cache-control'] || '').toString());
		}catch(_){
			reqCacheControl = new Map;
		}
		try{
			var resCacheControl = parseCacheControl((res.headers['cache-control'] || '').toString());
		}catch(_){
			resCacheControl = new Map;
		}
		if(reqCacheControl.has('no-store') || resCacheControl.has('no-store')) return;
		if(route.public && res.headers['authorization']) return;
		if(res.headers['expires'] || resCacheControl.has('max-age') || resCacheControl.has('s-maxage') || resCacheControl.has('public')){
...
ResourceCacheHit.prototype.handle = function handle(req){
	...
	const age = route.computeAge_s(obj.req, obj.res, req, obj.resDate, new Date);
	if(obj && obj.req.method===req.method && testVary(obj.req, obj.res, req)){
		obj.stream().pipe(res);
		res.addHeader('Age', age.toString());
