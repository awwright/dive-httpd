...
RouteStaticFile.prototype.prepareFromStat = function prepareFromStat(filepath, fileStat, match){
	var route = this;
	var Resource = this.resourceType;
	if(!match){
		match = route.pathResolve.resolveURI(filepath);
		if(!match) return Promise.resolve();
	}
	var uriMatch = match.rewrite(route.uriTemplate);
	var stream = fs.createReadStream(route.fileroot + filepath);
	...
	return Promise.resolve(new Resource(route, uriMatch, fileStat, stream));
};
RouteStaticFile.prototype.render = function render(resource){
	...
	if(this.filepathLink){
		...
		var linkHeader = '<file://'+this.filepathAuthority+resource.stream.path+'>;rel="'+this.filepathRel+'"'
		out.setHeader('Link', (out.getHeader('Link') || []).concat([ linkHeader ]));
	}
	return resource.stream.pipe(out);
}
...
RouteStaticFile.prototype.listDependents = function listDependents(){
	return [ {
		name: 'Filesystem',
		uriTemplate: this.fileroot+this.pathTemplate.template,
