...
RouteStaticFile.prototype.prepare = function prepare(match, euri, queryMap){
	if(typeof match==='string'){
		// Keep track of the outbound-facing URI Template
		// TODO initialize this when this.routerURITemplate does
		var routes = new TemplateRouter.Router();
		routes.addTemplate(this.routerURITemplate);
		match = routes.resolveURI(match);
	...
	// base should end with a / and path should begin with a /
	// normalize the path to remove the double-slash and any .. segments
	var filepath = pathNormalize(self.base + path);
	// Check that the file path is inside the supplied base path
	// This does not dereference symlinks, so use those with care
	if(filepath.substring(0, this.base.length) !== this.base){
		throw new Error('File requested is outside base directory');
	}
	return new Promise(function(resolve, reject){
		...
		stream.on('open', function(err){
			var rr = new self.resourceType(self, match, stream);
...
RouteStaticFile.prototype.watch = function watch(handler){
	...
	// If there's multiple watchers can some files be watched multiple times?
	...
	w.on('all', function(action, filepath, stat){
		...
		var result = self.pathResolve.resolveURI(filepath);
		// console.log('watch', self.base, filepath, self.path.template, result && result.data);
...
function ResourceStaticFile(route, match, stream){
	if(!(this instanceof ResourceStaticFile)) return new ResourceStaticFile(route, match, stream);
	this.route = route;
	this.uri = match.uri;
