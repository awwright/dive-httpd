...
function Application(opts){
	...
	this.onError = null;
	this.onReady = this.initialize(this.innerRoute.onReady || Promise.resolve());
}
...
Application.prototype.handleRequest = function handleRequest(req, res){
	...
	var prepareEvent = this.lock ? Promise.all(this.lock) : Promise.resolve();
	return prepareEvent.then(function(){
		return app.prepare(req.uri);
	}).then(function(resourceRequest){
		if(!resourceRequest && req.method==='PUT'){
			return app.store(req.uri);
		}
		...
		if(resource.methods.indexOf(req.method) === -1){
			return resource.error(req.uri, {statusCode: 405}).then(function(errorResource){
				if(errorResource){
...
Application.prototype.initialize = function initialize(ev){
	const self = this;
	if(!self.initializeLocks) self.initializeLocks = [];
	self.initializeLocks.push(ev);
	const complete = self.initializeComplete = Promise.all(self.initializeLocks);
	return self.initializeComplete.then(function(){
		if(self.initializeComplete!==complete) return self.initializeComplete;
	});
}
