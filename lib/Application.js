...
Application.prototype.handleRequest = function handleRequest(req, res){
	...
	if(req.headers['max-forwards']==='0'){
		if(req.method==='TRACE'){
			...
			res.statusCode = 200;
			new this.TraceResource(this).render(req).pipeMessage(res);
			return;
	...
	return this.initialize().then(function(){
		...
		if(resource){
			...
			return app.error(req.uri, notFoundError).then(function(errorResource){
				res.statusCode = 404;
				if(!errorResource && app.defaultNotFound){
					return app.defaultNotFound();
				}
				return errorResource;
			}).then(function(errorResource){
				if(req.method==='TRACE'){
					// TRACE in the case (2/3) that the resource does not exist
					if(errorResource && errorResource.trace){
						res.statusCode = 200;
						errorResource.trace(req).pipeMessage(res);
						return;
					}
					...
					return app.error(req.uri, new errors.NotImplemented({method:req.method})).then(function(notImplementedResource){
						if(notImplementedResource){
							notImplementedResource.render(req).pipeMessage(res);
						}else{
				...
				errorResource.render(req).pipeMessage(res);
