...
assert.isReadableResponse = function isReadableClientResponse(req, msg){
	assert(req instanceof stream.Stream, msg);
	// assert(req instanceof http.IncomingMessage);
	assert(typeof req.on === 'function', msg);
	assert(typeof req.read === 'function', msg);
};
